...
def uploadDocument():
    ...
    upload_files(session['username']+'/'+doc1,'maddysravs')
    return redirect('/documents')

@app.route('/documents')
def documents():
    s3_resource = boto3.resource("s3", region_name='us-east-1')
    s3_bucket = s3_resource.Bucket('maddysravs')
    data=[]
    ...
    # for obj in s3_bucket.objects.all():
        ...
        dummy.append('https://maddysravs.s3.amazonaws.com/'+obj.key)
        data.append(dummy)
    print(data)
...
def sharedDocuments():
    ...
    for i in k:
        ...
        if dummy['user']==session['email']:
            ...
            datai.append('https://maddysravs.s3.amazonaws.com/'+dummy['fileId'])
            data.append(datai)
    l=len(data)
    print(data)
